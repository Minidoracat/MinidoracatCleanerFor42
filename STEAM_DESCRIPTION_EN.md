...
[h2]⚠️ Requirement[/h2]
This mod requires the author's UI library [url=https://steamcommunity.com/sharedfiles/filedetails/?id=3789836701]Minidoracat UI Library for B42[/url] (listed as a required item). It only provides UI components and adds no game content; install it on both server and clients, and place it before this mod in the server's Mods list.
...
[*] [b]Three-dimensional animal control[/b]: [u]near players[/u] (counted within each player's scan radius), [u]server-wide[/u] (a distance-blind safety net for animals far from everyone yet still eating performance), and [u]per farm[/u] (counted per set of connected animal zones — the same unit the game uses to decide who can breed). Each dimension has its own limit, all overridable per species (e.g. "rat=20, chicken=80")
[*] [b]Farm breeding cap (gentle mode)[/b]: removes no living animal — past the cap it ends the newest pregnancies and turns the newest fertilised eggs back into ordinary eggs, keeping whatever is closest to birth. "The farm stops growing, nothing dies"
[*] [b]Ranches are safe[/b]: ranched animals are [u]never cleaned[/u] by default and animals inside hutches are never removed; named, held/leashed and hooked animals are absolutely protected under any configuration. Set a number only if you want a ceiling on your ranch
...
[*] [b]Dropper and handler tracking[/b]: records who last dropped each item and who last moved it between containers (with a timestamp), shown right in the item tooltip — no more guessing who littered or who took it. Times are shown in [u]each player's own timezone[/u], so the server needs no timezone setting
...
[*] [b]List manager[/b]: opened from the right-click menu — a sidebar switches between seven lists, search by keyword ([u]localized name or English ID both work[/u]), check results and add them in bulk with a value, batch-edit or remove entries below, select-all on both lists. Admins can [b]apply straight to the sandbox[/b] (using the game's own sandbox sync channel and permission check); regular players can still copy exact values for manual setup
...
[*] [b]Sandbox options[/b]: 24 settings split across three pages (General / Items / Animals), each with an explanation; defaults and per-species overrides share the same naming stem so they pair up at a glance; items and animals each get a [b]master on/off switch[/b]
...
[*] 50 stray animals per group; ranched removal and breeding are [b]not managed by default[/b] (limits 0)
